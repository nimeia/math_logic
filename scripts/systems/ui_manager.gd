...
func show_screen(scene_path: String) -> Control:
    ...
    if packed == null:
        Logger.error("UI screen not found: %s" % scene_path)
        return Control.new()
    var next := packed.instantiate() as Control
    if next == null:
        Logger.error("UI scene root must be Control: %s" % scene_path)
        return Control.new()
...
func show_difficulty_select(mode: String) -> void:
    ...
    if screen is Control:
        if screen.has_method("set_mode"):
            var mode_data := _mode_progress_data.get(mode, {})
...
func _on_request_quick_start(mode: String) -> void:
    if mode == MODE_NUMBERS:
        ...
        Logger.info("Quick start requested for mode: %s" % mode)

func _on_request_navigate(destination: String) -> void:
    Logger.info("Navigate to: %s" % destination)
...
func _on_difficulty_selected(mode: String, difficulty: String) -> void:
    if mode == MODE_NUMBERS:
        ...
        Logger.info("Difficulty selected: %s - %s" % [mode, difficulty])
...
func _preview_numbers_puzzle(mode: String, difficulty: String, reason: String) -> void:
    ...
    if puzzle.is_empty():
        Logger.error("Failed to generate puzzle for %s (%s)" % [mode, difficulty])
        return
    ...
    Logger.info("Generated %s puzzle via %s [%s]: %s -> answer %s" % [mode, reason, puzzle.get("template_id", ""), display, str(puzzle.get("answer", "?"))])
